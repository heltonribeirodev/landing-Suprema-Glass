...
// Valida que as variáveis obrigatórias existem
if (empty($_ENV['SENHA_HASH']) || empty($_ENV['TOKEN_SECRET'])) {
    header('Content-Type: application/json');
    http_response_code(500);
    echo json_encode(['ok' => false, 'erro' => 'Configuração do servidor incompleta. Verifique o arquivo .env']);
    exit;
}
...
if ($origin && (in_array($origin, $allowed_origins) || strpos($origin, 'localhost') !== false || strpos($origin, '127.0.0.1') !== false)) {
...
<<<<<<< HEAD
    ...
    // ── CONVERSÃO PARA WEBP ──────────────────────────────────────────────────
    // Verifica se a extensão GD com suporte a WebP está disponível
    if (!function_exists('imagewebp')) {
        // GD sem WebP: salva no formato original como fallback
        $ext_fallback = match($mime) {
            'image/png'  => 'png',
            'image/webp' => 'webp',
            default      => 'jpg',
        };
        $nome = $base . '-' . time() . '.' . $ext_fallback;
        $dest = IMG_PATH . $nome;
        if (!move_uploaded_file($file['tmp_name'], $dest)) {
            json_error(500, 'Falha ao salvar arquivo. Verifique permissões de assets/img/services/webp/');
        }
        json_ok(['url' => IMG_URL_BASE . $nome, 'nome' => $nome, 'aviso' => 'WebP não disponível no servidor, salvo no formato original.']);
    }

    // Carrega a imagem de acordo com o tipo de origem
    $imagem = match($mime) {
    ...
    if (!$imagem) {
        // Fallback: se não conseguir processar, salva original
        $ext_fallback = match($mime) { 'image/png' => 'png', 'image/webp' => 'webp', default => 'jpg' };
        $nome = $base . '-' . time() . '.' . $ext_fallback;
        $dest = IMG_PATH . $nome;
        move_uploaded_file($file['tmp_name'], $dest);
        json_ok(['url' => IMG_URL_BASE . $nome, 'nome' => $nome, 'aviso' => 'Imagem salva sem conversão.']);
    }
    ...
    // Converte e salva como WebP (qualidade 85)
    $ok = imagewebp($imagem, $dest, 85);
    imagedestroy($imagem);
